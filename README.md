Hi, I'm Fatima Mirani 👋
 
I am an MPhil Scholar in Linguistics and I bridge computational tools with qualitative language research. 

My work combines automated web scraping, corpus linguistics, and Natural Language Processing (NLP) to perform data-driven textual analysis at scale.

I use 'spaCy', `NLTK', Corpus Analysis Tools (AntConc, LancsBox) to do Critical Discourse Analysis (CDA) of large datasets to uncover systemic framing, agency, and ideological patterns.

I am also interested in Critical Digital Literacy and currently doing a project to observe patterns and meanings in Language Teacher-AI Interaction.

I'll be uploading some smaller projects, experiments, and things I am currently working on. 


# Tech & Analytical Stack

Languages: Python

Data & Scraping:'Selenium', 'BeautifulSoup', 'Pandas', 'Numpy'

Text Processing & NLP: 'spaCy', 'nltk', Corpus Analysis Tools (AntConc, LancsBox)

Methods: Critical Discourse Analysis (CDA), Corpus-Assisted Discourse Studies (CADS), Qualitative Coding







